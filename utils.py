...
def load_data(data_path, sfreq=256., ch_ind=[0, 1, 2, 3], stim_ind=5,
...
def plot_conditions(epochs, conditions=OrderedDict(), ci=97.5, n_boot=1000,
    ...
    for ch in range(4):
        for cond, color in zip(conditions.values(), palette):
            sns.lineplot(x=X[y.isin(cond), ch].mean(axis=0), y=times, color=color,
        ...
        axes[ch].set_xlim(times[0], times[-1])
